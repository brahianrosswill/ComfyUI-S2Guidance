...
- **Perpo-Guidance:** Instead of only subtracting the subnetwork, it computes the **orthogonal (perpendicular) component** relative to the CFG vector and subtracts that. Often yields sharper fine details or different stylistic contrasts. Test in the `0.1–0.5` range first.
...
Please note that not all DIT model architectures have been tested, but functionality has been validated on at least Chroma, Flux Klein, and Z-Image.
